...
<body>
  ...
  '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" class="ico-share-google"><span>google分享(另開視窗)</span></a></li>
                ...
                <div class="print-wrap">
                    <div class="page-header"><?php echo $result['history_story_title']?></div>
                    <div class="doc-detail">
                        <div class="date small text-muted">發布日期：<?php echo date('Y/m/d', strtotime($result['history_story_created_date']))?></div>
                        <!-- 文章內容 -->
                        <div class="article row-w-limit">
                            <?php echo stripslashes($result['history_story_content'])?>
                        </div>
                        <!-- end/ 文章內容 -->
                        <!-- 相關檔案 -->
                        <div class="article-footer">
                            <?php if(!empty($resultFile)){?>
                                ...
                                <?php foreach($resultFile as $file){?>
                                    <li><a href="<?php echo $filePath.$file['history_story_file_name']?>">檔案名稱： <?php echo $file['history_story_file_title']?></a></li>
                                <?php }?>
                                </ul>
                            <?php }?>
                            <div class="time-update">更新時間：<?php echo date('Y/m/d H:i:s', strtotime($result['history_story_edited_date']))?></div>
                        </div>
                        <!-- end/ 相關檔案 -->
                    </div>
                </div>
