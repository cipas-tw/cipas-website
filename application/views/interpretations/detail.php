...
<body>
  ...
  '', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;" class="ico-share-google"><span>google分享(另開視窗)</span></a></li>
                ...
                <div class="print-wrap">
                    <div class="page-header"><?php echo $result['rule_title']?></div>
                    <div class="doc-detail">
                        <div class="date small text-muted">發布日期：<?php echo date('Y/m/d', strtotime($result['rule_show_date']))?></div>
                        <!-- 文章內容 -->
                        <div class="article row-w-limit">
                            <?php echo stripslashes($result['rule_content'])?>
                        </div>
                        <!-- end/ 文章內容 -->
                        <!-- 相關檔案 -->
                        <div class="article-footer">
                            <?php if(!empty($resultFile)){?>
                                ...
                                <?php foreach($resultFile as $file){?>
                                    <li><a href="<?php echo $filePath.$file['rule_file_name']?>">檔案名稱： <?php echo $file['rule_file_title']?></a></li>
                                <?php }?>
                                </ul>
                            <?php }?>
                        </div>
